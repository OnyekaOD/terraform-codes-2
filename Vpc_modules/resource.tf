...
resource "aws_default_security_group" "rpost" {
...
resource "aws_vpc_dhcp_options" "rpost" {
...
resource "aws_vpc_dhcp_options_association" "rpost" {
  count = local.create_vpc && var.enable_dhcp_options ? 1 : 0
...
resource "aws_default_route_table" "default" {
  ...
  default_route_table_id = aws_vpc.rpost[0].default_route_table_id
...
resource "aws_internet_gateway" "rpost" {
...
resource "aws_route" "public_internet_gateway" {
  ...
  gateway_id             = aws_internet_gateway.rpost[0].id
...
resource "aws_route" "database_nat_gateway" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(aws_nat_gateway.rpost[*].id, count.index)
...
resource "aws_route" "database_ipv6_egress" {
  ...
  egress_only_gateway_id      = aws_egress_only_internet_gateway.rpost[0].id
...
resource "aws_subnet" "database" {
  ...
  ipv6_cidr_block = var.enable_ipv6 && length(var.database_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.rpost[0].ipv6_cidr_block, 8, var.database_subnet_ipv6_prefixes[count.index]) : null
...
resource "aws_default_network_acl" "rpost" {
  count = local.create_vpc && var.manage_default_network_acl ? 1 : 0

  default_network_acl_id = aws_vpc.rpost[0].default_network_acl_id
...
resource "aws_nat_gateway" "rpost" {
  ...
  depends_on = [aws_internet_gateway.rpost]
}
...
resource "aws_customer_gateway" "rpost" {
...
resource "aws_vpn_gateway" "rpost" {
...
resource "aws_vpn_gateway_attachment" "rpost" {
...
resource "aws_vpn_gateway_route_propagation" "public" {
  ...
  vpn_gateway_id = element(
    concat(
      aws_vpn_gateway.rpost[*].id,
      aws_vpn_gateway_attachment.rpost[*].vpn_gateway_id,
    ),
    count.index,
  )
}
...
resource "aws_default_vpc" "rpost" {
