locals {
  ...
  password               = local.create_random_password ? random_password.rpost_password[0].result : var.password
...
resource "random_password" "rpost_password" {
...
resource "aws_kms_key" "performance_insights_kms_key" {
  description = "KMS key is used to encrypt bucket objects"
}

resource "aws_kms_key" "rpost_key" {
  description             = "KMS key is used to encrypt bucket objects"
